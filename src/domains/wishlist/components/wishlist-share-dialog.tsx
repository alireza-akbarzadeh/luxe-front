...
import { buildWishlistShareUrl, encodeWishlistShare } from '@/domains/wishlist/lib/wishlist-share';
...
export function WishlistShareDialog({
  ...
  onOpenChange,
  productIds
}: Readonly<WishlistShareDialogProps>) {
  const t = useTranslations('wishlist.share');
  const [copiedKey, setCopiedKey] = useState<'link' | 'code' | null>(null);

  const code = encodeWishlistShare(productIds);
  const shareUrl = code ? buildWishlistShareUrl(code) : '';
