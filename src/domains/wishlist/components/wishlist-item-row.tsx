      ...
      <Flex direction='column' gap={2} className='min-w-0 flex-1'>
        <Flex align='start' justify='between' gap={2}>
          ...
          >
            {isRemoving ? (
              ...
              <IconHeartFilled className='text-destructive size-4 fill-current' />
            )}
          </button>
        </Flex>

        <Flex align='center' gap={2} wrap='wrap'>
          <Typography.Text weight='semibold' className='tabular-nums'>
          ...
          {hasDiscount && item.discount_percent ? (
            <span className='bg-emerald-600/10 text-emerald-700 dark:text-emerald-400 rounded-full px-2 py-0.5 text-[10px] font-semibold'>
        ...
        {productQuantity > 0 ? (
          <Flex
            align='center'
            justify='between'
            className='bg-muted/80 h-10 rounded-full border px-1'
          >
            <Button
              type='button'
              variant='ghost'
