...
import { PlusActivationSuccessBanner } from '@/domains/account/components/plus-activation-success-banner';
...
export function PlusMembershipPanel() {
  ...
  const activationReceipt = usePlusActivationStore((state) => state.receipt);

  return (
    <>
      {activationReceipt ? <PlusActivationSuccessBanner receipt={activationReceipt} /> : null}
      <Box className='border-border from-gold/5 via-card to-card rounded-2xl border bg-linear-to-br p-6'>
        ...
        {isPlus && membership?.plus_expires_at ? (
          <Text variant='muted' className='mb-4 text-sm'>
            {membership.plus_subscribed_at
              ? t('subscribedOn', {
                  date: new Date(membership.plus_subscribed_at).toLocaleDateString()
                })
              : null}
            {membership.plus_subscribed_at ? ' · ' : null}
