...
import { useGetPublicCollections } from '@/services/-public-collections-get';
import type { GetPublicCollections200 } from '@/services/-public-collections-get.schemas';

import { PublicCollectionCard } from './components/public-collection-card';

export function PublicCollectionsDomain() {
  const t = useTranslations('publicCollectionsPage');
  const { data: listResponse, isLoading } = useGetPublicCollections(
    { limit: 12 },
    { query: { staleTime: 60_000 } }
  );

  const listData = listResponse as GetPublicCollections200 | undefined;
          ...
          <Grid gap={6} className='mt-12 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
            {collections.map((collection, index) => (
              <PublicCollectionCard
                ...
                itemsLabel={t('itemCount', { count: collection.item_count ?? 0 })}
              />
            ))}
          </Grid>
        ) : (
