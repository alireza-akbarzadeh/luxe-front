...
export function ProductReviewAiSummary({ productId, reviewCount }: ProductReviewAiSummaryProps) {
  ...
  const { ref, visible } = useWhenVisible<HTMLDivElement>({ rootMargin: '200px 0px' });
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- visible + productId gate fetch
  }, [productId, reviewCount, qualifies, visible]);
  ...
  if (!visible) {
    return (
      <div
        ref={ref}
        className='border-gold/25 min-h-[10rem] rounded-2xl border border-dashed'
        aria-hidden
      />
    );
  }
  ...
  return (
    <div ref={ref}>
      <Card className='border-gold/25 from-gold/5 to-card min-h-[10rem] rounded-2xl border bg-linear-to-br p-5 sm:p-6'>
