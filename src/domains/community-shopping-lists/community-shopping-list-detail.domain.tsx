...
import { useGetCommunityListsSlug } from '@/services/-community-lists-{slug}-get';
import type { GetCommunityListsSlug200 } from '@/services/-community-lists-{slug}-get.schemas';
import type { DtoProductWithLike } from '@/services/-products-get.schemas';

type CommunityShoppingListDetailDomainProps = {
  slug: string;
};

export function CommunityShoppingListDetailDomain({
  slug
}: CommunityShoppingListDetailDomainProps) {
  const t = useTranslations('communityListsPage');
  const {
    data: response,
    isLoading,
    isError
  } = useGetCommunityListsSlug(slug, {
    query: { staleTime: 60_000 }
  });

  const payload = response as GetCommunityListsSlug200 | undefined;
      ...
      <main className='pb-24'>
        <div className='app-container pt-24'>
          ...
          <Button asChild className='mt-6'>
            <Link href='/lists'>{t('backToList')}</Link>
          </Button>
        </div>
      </main>
    );
  ...
  return (
    <main className='pb-24'>
      <div className='app-container pt-24'>
        <DynamicBreadcrumb
          items={[{ label: t('breadcrumb'), href: '/lists' }, { label: list.title ?? slug }]}
          direction='column'
        ...
        <div className='relative mt-10 overflow-hidden rounded-3xl'>
          ...
          <Flex direction='column' gap={2} className='absolute inset-x-0 bottom-0 px-6 pb-6'>
            {list.theme ? (
              <Typography.Overline className='text-gold'>{list.theme}</Typography.Overline>
            ) : null}
            ...
            {list.author_name ? (
              <Typography.P className='text-white/80'>
                {t('curatedBy', { name: list.author_name })}
