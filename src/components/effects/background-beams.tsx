'use client';

import { motion, useReducedMotion } from 'framer-motion';
import React from 'react';

import { cn } from '@/lib/utils';

const ALL_BEAM_PATHS = [
  'M-380 -189C-380 -189 -312 216 152 343C616 470 684 875 684 875',
  'M-352 -221C-352 -221 -284 184 180 311C644 438 712 843 712 843',
  'M-324 -253C-324 -253 -256 152 208 279C672 406 740 811 740 811',
  'M-296 -285C-296 -285 -228 120 236 247C700 374 768 779 768 779',
  'M-268 -317C-268 -317 -200 88 264 215C728 342 796 747 796 747',
  'M-240 -349C-240 -349 -172 56 292 183C756 310 824 715 824 715',
  'M-212 -381C-212 -381 -144 24 320 151C784 278 852 683 852 683',
  'M-184 -413C-184 -413 -116 -8 348 119C812 246 880 651 880 651',
  'M-156 -445C-156 -445 -88 -40 376 87C840 214 908 619 908 619',
  'M-128 -477C-128 -477 -60 -72 404 55C868 182 936 587 936 587',
  'M-100 -509C-100 -509 -32 -104 432 23C896 150 964 555 964 555',
  'M-72 -541C-72 -541 -4 -136 460 -9C924 118 992 523 992 523'
] as const;

/** Stable per-path animation params — avoids Math.random() on every render. */
const BEAM_ANIMATIONS = ALL_BEAM_PATHS.map((_, index) => {
  const seed = (index * 17 + 7) % 100;
  return {
    y2End: `${93 + (seed % 8)}%`,
    duration: 10 + (seed % 10),
    delay: (seed % 10) * 0.5
  };
});

export const BackgroundBeams = React.memo(({ className }: { className?: string }) => {
  const reduceMotion = useReducedMotion();

  if (reduceMotion) {
    return null;
  }

  return (
    <div
      className={cn(
        'absolute inset-0 flex h-full w-full items-center justify-center [mask-size:40px] [mask-repeat:no-repeat]',
        className
      )}
    >
      <svg
        className='pointer-events-none absolute z-0 h-full w-full'
        width='100%'
        height='100%'
        viewBox='0 0 696 316'
        fill='none'
        xmlns='http://www.w3.org/2000/svg'
        aria-hidden
      >
        {ALL_BEAM_PATHS.map((path, index) => (
          <motion.path
            key={`beam-path-${index}`}
            d={path}
            stroke={`url(#beam-gradient-${index})`}
            strokeOpacity='0.35'
            strokeWidth='0.5'
          />
        ))}
        <defs>
          {ALL_BEAM_PATHS.map((_path, index) => {
            const anim = BEAM_ANIMATIONS[index];
            return (
              <motion.linearGradient
                id={`beam-gradient-${index}`}
                key={`beam-gradient-${index}`}
                initial={{ x1: '0%', x2: '0%', y1: '0%', y2: '0%' }}
                animate={{
                  x1: ['0%', '100%'],
                  x2: ['0%', '95%'],
                  y1: ['0%', '100%'],
                  y2: ['0%', anim?.y2End ?? '95%']
                }}
                transition={{
                  duration: anim?.duration ?? 12,
                  ease: 'easeInOut',
                  repeat: Infinity,
                  delay: anim?.delay ?? 0
                }}
              >
                <stop stopColor='var(--gold)' stopOpacity='0' />
                <stop stopColor='var(--gold)' />
                <stop offset='45%' stopColor='var(--gold-strong)' />
                <stop offset='100%' stopColor='var(--gold)' stopOpacity='0' />
              </motion.linearGradient>
            );
          })}
        </defs>
      </svg>
    </div>
  );
});

BackgroundBeams.displayName = 'BackgroundBeams';
