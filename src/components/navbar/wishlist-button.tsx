...
export function WishlistButton() {
  ...
  return (
    <Button
      ...
      aria-label={`Wishlist${itemCount > 0 ? `, ${itemCount} saved items` : ''}`}
    >
      <Link href='/wishlist'>
        ...
        <AnimatePresence mode='popLayout'>
          {isAuthenticated && itemCount > 0 ? (
            <motion.span
              layout
              className='bg-gold text-gold-foreground ring-background absolute top-1 right-1 flex h-4 min-w-4 items-center justify-center rounded-full px-0.5 text-[10px] font-bold shadow-sm ring-2'
