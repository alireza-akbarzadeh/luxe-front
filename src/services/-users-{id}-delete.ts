...
import type { UtilsResponse } from './-users-{id}-delete.schemas';
...
export const deleteUsersId = (
  ...
  return customInstance<UtilsResponse>({ url: `/users/${id}`, method: 'DELETE', signal }, options);
};
...
> => {
  ...
  const mutationFn: MutationFunction<Awaited<ReturnType<typeof deleteUsersId>>, { id: number }> = (
    props
  ) => {
    const { id } = props ?? {};
 ...
 * @summary Delete a user
 */
export const useDeleteUsersId = <TError = UtilsResponse, TContext = unknown>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof deleteUsersId>>,
      TError,
